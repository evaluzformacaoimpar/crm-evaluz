...
(function () {
  ...
  async function init() {
    ...
    if (new URLSearchParams(window.location.search).get('novo') === '1') {
      var btnAuto = document.getElementById('btn-novo-contato');
  ...
  function bindEvents() {
    ...
    var btnExport = document.getElementById('btn-exportar');
  ...
  async function loadContacts() {
    ...
    try {
      ...
      // Carrega data do último contato para todos os contatos
      try {
        ...
        if (!histRes.error && histRes.data) {
          ...
          histRes.data.forEach(function(h) {
            if (!lastContactedMap[h.contact_id]) {
              lastContactedMap[h.contact_id] = h.created_at;
            }
          });
        }
      } catch(e) {}
  ...
  function renderLista(contacts) {
    ...
    if (!contacts.length) {
      tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--text-muted);">Nenhum contato encontrado</td></tr>';
      return;
    }
    var html = '';
    contacts.forEach(function(c) {
      var discTag      = c.disc_profile && DISC_DATA[c.disc_profile] ? DISC_DATA[c.disc_profile].tag : '';
      var lastContact  = lastContactedMap[c.id] || null;
      var lastStr      = lastContact ? formatDate(lastContact) : '<span style="color:var(--text-muted);font-size:0.78rem;">Nunca</span>';
      var diasSem      = '';
      if (lastContact) {
        var diff = Math.floor((Date.now() - new Date(lastContact)) / 86400000);
        if (diff > 7) diasSem = '<span style="color:var(--danger);font-size:0.72rem;display:block;">' + diff + ' dias sem contato</span>';
      }
      html += '<tr>';
  ...
  function exportarCSV() {
    var filtered = getFiltered();
    if (!filtered.length) { showToast('Nenhum contato para exportar', 'error'); return; }

    var header = ['Nome','Telefone','Email','Instagram','Etapa','Origem','DISC','Tipo Cliente','Ultimo Contato','Data Cadastro'];
    var rows = filtered.map(function(c) {
      var last = lastContactedMap[c.id] ? formatDate(lastContactedMap[c.id]) : '';
      return [
        c.name        || '',
        c.phone       || '',
        c.email       || '',
        ...
        getDiscLabel(c.disc_profile)   || '',
        getClientTypeLabel(c.client_type) || '',
        last,
    ...
    link.download = 'contatos_evaluz_' + todayISO() + '.csv';
  ...
  // ── Delegação de eventos (cliques em cards/linhas) ────────────
  ...
  document.addEventListener('click', function(e) {
    ...
    // Clique no card (não em botão) — abre detalhe
  ...
  // Clique no card kanban (fora dos botões de ação)
  document.addEventListener('click', function(e) {
    var card = e.target.closest('.kanban-card');
    if (!card) return;
  ...
  async function salvarContato() {
    ...
    // Criar tarefa de agendamento se solicitado
    var contatoId = id;
